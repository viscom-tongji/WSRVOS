...
except ImportError:  # torchvision video ops require PyAV at runtime
    read_video = None
import h5py
...
def _read_video_frames(video_path):
    ...
    if not capture.isOpened():
        raise RuntimeError(f'error: failed to open video file: {video_path}')
    ...
    if not frames:
        raise RuntimeError(f'error: failed to decode any frames from video: {video_path}')
...
class A2DSentencesDataset(Dataset):
    ...
    def __init__(self, subset_type: str = 'train', dataset_path: str = 'data/a2d_sentences', window_size=8,
        ...
        if subset_type == 'test' and dataset_coco_gt_format_path and not path.exists(dataset_coco_gt_format_path):
    ...
    def __getitem__(self, idx):
        ...
        video_frames = _read_video_frames(path.join(self.videos_dir, f'{video_id}.mp4'))
